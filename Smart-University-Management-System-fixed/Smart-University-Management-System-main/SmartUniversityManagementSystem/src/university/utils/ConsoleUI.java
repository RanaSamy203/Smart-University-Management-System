...
public class ConsoleUI {
    ...
    private void doAddStudent() {
        ...
        double gpa          = input.readDoubleInRange("GPA (0.0-4.0): ", 0.0, 4.0);
        int    level        = input.readIntInRange("Academic Level (1-4): ", 1, 4);
    ...
    private void doAddProfessor() {
        ...
        if (studentService.addProfessor(prof)) {
            System.out.println("-> Professor added successfully.");
        } else {
            System.out.println("-> A professor with ID " + id + " already exists.");
        }
    }
    ...
    private void doDeleteProfessor() {
        ...
        if (confirm) {
            if (studentService.deleteProfessor(id)) {
    ...
    private void doRegisterStudent() {
        ...
        try {
            ...
            if (enrollmentService.registerStudent(s, c)) {
    ...
    private void doDropCourse() {
        ...
        try {
            ...
            if (enrollmentService.dropCourse(s, c)) {
    ...
    private void doMakePayment() {
        ...
        try {
            ...
            // Throws InvalidPaymentException on failure, so the success
            // message below is only printed when the payment really went through.
